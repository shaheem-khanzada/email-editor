  ...
  <div class="tree">
    <ul class="tree-list" :style="{ gap: `${gap}px` }">
      <template v-for="node in filteredData" :key="node.id">
        <tree-row v-if="!node.hidden" :ref="`tree-row-${node.id}`" :node="node" :selectedFolderId="selectedFolderId"
          :use-checkbox="useCheckbox" :use-icon="useIcon" :use-row-delete="useRowDelete"
          :show-child-count="showChildCount" :indent-size="indentSize" :gap="gap"
          :row-hover-background="rowHoverBackground" :set-node="setNode" :get-node="getNode" :update-node="updateNode"
          :expandable="expandable" @delete-row="onDeleteRow" @node-click="onNodeClick" @node-expanded="onNodeExpanded"
          @toggle-checkbox="onToggleCheckbox">
          <template #checkbox="{ node: slotNode, checked, indeterminate }">
            <slot name="checkbox" :node="slotNode" :checked="checked" :indeterminate="indeterminate"
